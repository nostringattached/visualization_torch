local ModifiedReLU, parent = torch.class('nn.ModifiedReLU','nn.Module')

function ModifiedReLU:__init(th,v,ip)
   parent.__init(self)
   self.threshold = th or 1e-6
   self.val = v or 0
   if (th and type(th) ~= 'number') or (v and type(v) ~= 'number') then
      error('nn.ModifiedReLU(threshold, value)')
   end
...
function ModifiedReLU:updateOutput(input)
   self:validateParameters()
...
function ModifiedReLU:updateGradInput(input, gradOutput)
   self:validateParameters()
...
function ModifiedReLU:validateParameters()
